...
- ошибка валидации данных (ValidationException) - код ответа: 400 Bad Request;
- ошибка валидации аргументов метода (MethodArgumentNotValidException) - код ответа: 400 Bad Request;
- ошибка поиска объекта (NotFoundException) - код ответа: 404 Not Found;
- внутренняя ошибка сервера (Throwable) - код ответа: 500 Internal Server Error.
...
Для работы с проектом требуется JDK версии 21.
...
Установите Homebrew, если он еще не установлен. Откройте терминал и выполните команду:
...
Для сборки и управления проектом используется Apache Maven версии 3.9.9.
...
1. Скачайте установочный архив Maven 3.9.9 с официального сайта: Apache Maven 3.9.9.
...
Для работы с приложением необходимо убедиться, что используемый порт свободен.
...
*Примечание: нижеуказанные команды следует выполнять в терминале (командной строке) операционной системы*
